 ...
 *
 * Lives in packages/shared (not apps/web) because services/nr-ingest needs the
 * same real-instant resolution to time-check a TD headcode's report against a
 * candidate Darwin schedule — see findRidForHeadcode's use of hhmmToIso.
 * apps/web/lib/uk-time.ts re-exports everything here for existing call sites.
 *
 ...
 * refinement pass.
 */
function londonWallToInstant(year: number, month: number, day: number, minutesOfDay: number): Date {
 ...
 */
export function hhmmToIso(hhmm: string | null | undefined, anchor: Date = new Date()): string | undefined {
