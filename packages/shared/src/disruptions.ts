 ...
 *
 * Framework-free (fetch + string parsing only) so both apps/web (the /status
 * board) and services/darwin-ingest (matching disruptions to commutes for
 * push alerts) can use it. apps/web/lib/disruptions.ts re-exports this —
 * there is exactly one implementation.
 */

import { tocRegion } from "./toc.js";
 ...
 * operator name to the Disruptions API's tocName for those. Originally lived
 * in apps/web/app/status/page.tsx; moved here alongside the client it
 * supports so darwin-ingest's commute-matching can reuse it too.
 */
export const RTPPM_TO_DISRUPTIONS_NAME: Record<string, string> = {
